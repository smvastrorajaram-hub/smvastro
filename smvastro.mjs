...
// FAST CHECKOUT WARM-UP: start network/TLS and wake the payment backend before Pay is clicked.
// This changes no payment timeout, retry rule, amount, offer, or verification logic.
let smvPaymentWarmAt=0;
function warmPaymentPath(){
 const now=Date.now(); if(now-smvPaymentWarmAt<45000)return; smvPaymentWarmAt=now;
 try{fetch(RAZORPAY_BACKEND_URL+"/",{method:"GET",mode:"cors",cache:"no-store",keepalive:true}).catch(()=>{});}catch(_e){}
 try{ensureRazorpayCheckout().catch(()=>{});}catch(_e){}
}
if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",()=>setTimeout(warmPaymentPath,250),{once:true});
else setTimeout(warmPaymentPath,250);
window.addEventListener("focus",warmPaymentPath);
document.addEventListener("pointerdown",e=>{if(e.target?.closest?.("#askNowButton,#submitQuestionBtn,#privateConsultPayBtn,#privateConsultationHomeLink,[data-private-retry],[data-retry-payment]"))warmPaymentPath();},{passive:true});
...
$("submitQuestionBtn")?.addEventListener("click",async()=>{
 ...
 try{
  ...
  // and explicitly tagged as Asia/Kolkata; they are NOT converted through UTC.
   ...
   const clientQuestionId=makeQuestionId();
   const payload={questionId:clientQuestionId,customerName:name,question:text,amount,birthDetails:{name,birthDate,birthTime,birthPlace,birthGender,timezone:"Asia/Kolkata",utcOffsetMinutes:330},serviceName:"Public Astrology Question",customerEmail:currentUser.email||"",promoCode:String($("smvPromoCode")?.value||"").trim()};
