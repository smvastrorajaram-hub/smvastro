...
async function renderAdminOffers(){
  ...
  async function reload(){const r=await renderApi('/admin/offers',{method:'GET'}),list=$("adminOfferList"),offers=r.offers||[];list.innerHTML=offers.length?offers.sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''))).map(o=>`<div class="card" style="margin-top:8px"><b>${escapeHtml(o.name||'Offer')}</b> ${o.enabled?'<span class="success">ON</span>':'<span class="small">OFF</span>'}<div class="small">${escapeHtml(o.promoCode||'Automatic')} · ${escapeHtml(o.discountType||'fixed_price')} · Used ${Number(o.usedCount||0)}${Number(o.totalUsageLimit||0)>0?' / '+Number(o.totalUsageLimit):''} · Successful Payments ${Number(o.successfulPayments||0)} · Discount Given ₹${Number(o.totalDiscountGiven||0).toFixed(2)}</div><div class="action-row"><button class="btn" data-offer-edit="${escapeHtml(o.id)}">EDIT</button>${o.builtIn?'':'<button class="btn gray" data-offer-delete="'+escapeHtml(o.id)+'">DELETE</button>'}</div></div>`).join(''):'<div class="empty">No offers created yet.</div>';list.querySelectorAll('[data-offer-edit]').forEach(b=>b.onclick=()=>{const o=offers.find(x=>x.id===b.dataset.offerEdit);if(!o)return;$('offerEditId').value=o.id;$('offerName').value=o.name||'';$('offerPromoCode').value=o.promoCode||'';$('offerDiscountType').value=o.discountType||'fixed_price';$('offerPrice').value=o.offerPrice??1;$('offerDiscountValue').value=o.discountValue??0;$('offerEligibility').value=o.eligibility||'all';$('offerAppliesTo').value=(o.appliesTo||['public_question'])[0]||'public_question';$('offerDisplayMode').value=o.displayMode||'payment_only';$('offerStartAt').value=typeof o.startAt==='string'?o.startAt.slice(0,16):'';$('offerEndAt').value=typeof o.endAt==='string'?o.endAt.slice(0,16):'';$('offerPerCustomerLimit').value=o.perCustomerLimit??1;$('offerTotalUsageLimit').value=o.totalUsageLimit??0;$('offerMinimumAmount').value=o.minimumAmount??1;$('offerAutomatic').value=String(o.automatic!==false);$('offerEnabled').value=String(o.enabled===true);$('offerBannerText').value=o.bannerText||'';card.scrollIntoView({behavior:'smooth',block:'start'});});list.querySelectorAll('[data-offer-delete]').forEach(b=>b.onclick=async()=>{if(!confirm('Delete this offer?'))return;await renderApi('/admin/offers/delete',{method:'POST',body:JSON.stringify({id:b.dataset.offerDelete})});await reload();});}
  $("saveOfferBtn").onclick=async()=>{const b=$("saveOfferBtn");b.disabled=true;try{const applies=$('offerAppliesTo').value;await renderApi('/admin/offers/save',{method:'POST',body:JSON.stringify({id:$('offerEditId').value||undefined,name:$('offerName').value,promoCode:$('offerPromoCode').value,discountType:$('offerDiscountType').value,offerPrice:Number($('offerPrice').value),discountValue:Number($('offerDiscountValue').value),eligibility:$('offerEligibility').value,appliesTo:applies==='all'?['all']:[applies],displayMode:$('offerDisplayMode').value,startAt:$('offerStartAt').value||null,endAt:$('offerEndAt').value||null,perCustomerLimit:Number($('offerPerCustomerLimit').value),totalUsageLimit:Number($('offerTotalUsageLimit').value),minimumAmount:Number($('offerMinimumAmount').value),automatic:$('offerAutomatic').value==='true',enabled:$('offerEnabled').value==='true',bannerText:$('offerBannerText').value})});$('adminOfferMsg').innerHTML='<span class="success">Offer saved.</span>';clear();await reload();}catch(e){$('adminOfferMsg').innerHTML='<span class="error">'+escapeHtml(e.message||String(e))+'</span>';}finally{b.disabled=false;}};try{await reload();}catch(e){$("adminOfferList").innerHTML='<div class="error">'+escapeHtml(e.message||String(e))+'</div>';}
}
