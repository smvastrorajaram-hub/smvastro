...
const RAZORPAY_BACKEND_URL=window.SMV_CONFIG.backendUrl;
...
function hideHomeSurface(){
  ...
  ["home","askNowSection","approved-astrologers","faq",
   "smv-content-hub","horoscope","horoscope-tools","tamil-horoscope","english-horoscope"].forEach(id=>hide(id));
...
function smvReturnHome(restoreRoleNav=true){
  ...
  ["home","askNowSection","approved-astrologers","faq","smv-content-hub",
   "horoscope","tamil-horoscope","english-horoscope"].forEach(id=>{const el=$(id);if(el)el.classList.remove("hidden");});
...
window.closeModal=()=>{
  ...
  if(m){
    ...
    m.removeAttribute("aria-labelledby"); m.removeAttribute("aria-modal"); m.removeAttribute("role");
  }
...
$("contentNav")?.addEventListener("click",(e)=>{
  ...
  requestAnimationFrame(()=>{
    const target=$("horoscope");
    if(target) target.scrollIntoView({behavior:"smooth",block:"start"});
  });
});

// Both Home links are bound once by the common header navigation above.
...
function openAuth(mode="login"){
  ...
  openModal(`<div class="auth-shell">
    <div class="auth-hero"><h2 id="authTitle">${isLogin?"Welcome Back":"Create Account"}</h2><p>${isLogin?"Sign in to continue to your SMV ASTRO dashboard.":"Create your secure customer account."}</p></div>
    ...
    <label class="auth-field-label" for="email">${isLogin?"Email / ID":"Email"}</label><input class="auth-field" id="email" type="email" placeholder="Email" autocomplete="email">
    <label class="auth-field-label" for="password">Password</label><div class="password-wrap"><input class="auth-field" id="password" type="password" placeholder="Minimum 6 characters" autocomplete="${isLogin?"current-password":"new-password"}"><button type="button" class="password-toggle" id="passwordToggle" aria-label="Show password">◉</button></div>
  ...
  $("modal").setAttribute("role","dialog"); $("modal").setAttribute("aria-modal","true"); $("modal").setAttribute("aria-labelledby","authTitle");
  ...
  if(isLogin){
    const setMethod=(m)=>{loginMethod=m; const input=$("email"); if(!input)return; const idMode=m!=="email"; input.type=idMode?"text":"email"; input.placeholder=m==="customerId"?"Customer ID (e.g. SMV-CUS-20082026-01)":m==="astrologerId"?"Astrologer ID (e.g. SMV-AST-20082026-01)":"Email address"; input.autocomplete=idMode?"username":"email"; $("loginEmailMode")?.classList.toggle("active",m==="email");
      [["loginEmailMode","email"],["loginCustomerIdMode","customerId"],["loginAstrologerIdMode","astrologerId"]].forEach(([id,key])=>$(id)?.setAttribute("aria-pressed",String(key===m))); $("loginCustomerIdMode")?.classList.toggle("active",m==="customerId"); $("loginAstrologerIdMode")?.classList.toggle("active",m==="astrologerId");};
...
async function logoutToHome(reason=''){
  ...
  show('horoscope'); show('tamil-horoscope'); show('english-horoscope'); window.__smvContentVisible=false;
...
async function loadDashboard(expectedRole=null,force=false){
 ...
 if(!force && dashboardReadyUid===loadUid && (!requestedRole || dashboardReadyRole===requestedRole) && smvInternalView==='dashboard' && box && !box.querySelector('.error')){
   show('dashboard');
   touchSession();
   armIdleTimer();
   return;
...
const userStatus=String(
  ...
  return;
   ...
   const availableQuestions=inboxSnap.docs.filter(d=>{const q=d.data()||{};return q.astrologerId===currentUser.uid&&!!q.adminQuestionApprovedAt&&['assigned_to_astrologer','available_to_astrologers','reallocated'].includes(String(q.allocationStatus||''))&&['admin_approved','paid'].includes(String(q.status||''));}).map(d=>({id:d.id,...d.data()}));
   try{
...
<div class="card" style="margin-top:16px"><h3>Payment Method</h3><p class="small">Your bank/UPI details are private. Full details are not displayed again.</p><button class="btn gray" id="changePayoutBtn2">Change Payment Method</button></div>`;
  ...
  try {
   ...
   // Reconcile pending Razorpay refunds before rendering the card. The webhook is
   // the primary path; this lightweight sync is a fallback when the webhook is
   // delayed or not configured. Only the customer's own rejected questions are sent.
  ...
  try{ await renderNotifications('userNotifications'); }
  ...
  if(!active()) return;
  show('dashboard');
  dashboardReadyUid=loadUid;
  dashboardReadyRole=role;
  touchSession();
  armIdleTimer();
 }catch(e){
   if(!active()) return;
 ...
 try{
  ...
  try{
    const pendingQuestions=questions.docs.filter(d=>{
      const q=d.data()||{};
      const waitingApproval = q.status==='pending_admin_approval' || (q.status==='paid' && !q.adminQuestionApprovedAt);
      const allocatedWaiting = !!q.adminQuestionApprovedAt && !q.answer && !['answered','question_rejected'].includes(String(q.status||'')) && ['assigned_to_astrologer','reallocated','available_to_astrologers','claimed_by_astrologer'].includes(String(q.allocationStatus||''));
      return waitingApproval || allocatedWaiting;
    });
    ...
    questionApprovalBox.innerHTML=pendingQuestions.length ? pendingQuestions.map(d=>{
      ...
      const allocated = !!q.adminQuestionApprovedAt && !q.answer && ['assigned_to_astrologer','reallocated','available_to_astrologers','claimed_by_astrologer'].includes(String(q.allocationStatus||''));
      const currentAstroName = q.astrologerName || q.astrologerId || 'Not assigned';
      return `<div class="card" style="margin:10px 0"><h3>${escapeHtml(customerName)}</h3><p><b>Question ID:</b> ${escapeHtml(d.id)}</p><p><b>Question:</b> ${escapeHtml(q.question||'')}</p><p><b>Birth:</b> ${escapeHtml(birthDate)} · ${escapeHtml(birthTime)} · ${escapeHtml(birthPlace)} · ${escapeHtml(birthGender)}</p><p><b>Amount Paid:</b> ₹${Number(q.amount||0).toFixed(2)} · <b>Payment ID:</b> ${escapeHtml(q.customerPaymentId||'')} · <b>Paid:</b> ${escapeHtml(smvDateTime(q.paymentRecordedAt||q.paidAt||q.paymentUpdatedAt||q.paymentDate))}</p>${allocated ? `<div class="small"><b>Status:</b> Approved & Allocated · <b>Astrologer:</b> ${escapeHtml(currentAstroName)}</div><div class="action-row" style="margin-top:10px"><button class="btn gray" data-board-keep="${d.id}">KEEP ALLOCATED</button><select id="boardReallocAstro_${d.id}"><option value="">Select another approved astrologer</option>${approvedAstros.filter(a=>a.id!==q.astrologerId).map(a=>{const x=a.data()||{};return `<option value="${escapeHtml(a.id)}">${escapeHtml(x.name||'Astrologer')} — ${escapeHtml(x.expertise||x.specialization||'Astrology')}</option>`}).join('')}</select><input id="boardReallocPct_${d.id}" type="number" min="0" max="100" step="0.01" value="${Number(q.commissionPercent||q.commissionRate||settings.astroPercent||20)}" placeholder="Commission %"><button class="btn gray" data-board-realloc="${d.id}">RE-ALLOCATE</button><button class="btn" data-board-admin-answer="${d.id}">ADMIN ANSWER</button></div><textarea id="boardAdminAnswer_${d.id}" style="min-height:120px;margin-top:10px" placeholder="Admin answer — use only if the allocated astrologer is not answering."></textarea><div class="small">Question stays in this Admin board until the final answer is submitted and approved.</div>` : `<div class="grid" style="margin-top:10px"><div><label>Assign Astrologer</label><select id="assignAstro_${d.id}"><option value="">Select approved astrologer</option>${options}</select></div><div>`}<label>Astrologer Commission %</label><input id="assignCommission_${d.id}" type="number" min="0" max="100" step="0.01" value="${pct}"></div></div><p class="small" id="assignPreview_${d.id}"></p><div class="action-row"><button class="btn" data-approve-question="${d.id}">APPROVE & ALLOCATE</button><button class="btn gray" data-reject-question="${d.id}">REJECT QUESTION</button></div><input id="questionReject_${d.id}" placeholder="Rejection reason (required if rejecting)"></div>`;
  ...
  questionApprovalBox.querySelectorAll('[data-approve-question]').forEach(b=>b.onclick=async()=>{
    ...
    const astroId=$('assignAstro_'+id)?.value||'';
    const pct=Number($('assignCommission_'+id)?.value);
    ...
    const astroDoc=astros.docs.find(x=>x.id===astroId);
    const astro=astroDoc?.data()||{};
    b.disabled=true; b.textContent='APPROVING...';
    try {
      // First use the trusted Admin endpoint for the normal allocation/commission flow.
      const result=await renderApi('/admin/approve-question',{
        method:'POST',
        body:JSON.stringify({questionId:id,astrologerId:astroId,commissionPercent:pct})
      });
      if(!result?.success) throw new Error(result?.error||'Unable to approve and allocate question.');

      // CRITICAL CUSTOMER SYNC FIX:
      // The Customer dashboard must receive an explicit approval marker.
      // Do this after the Admin endpoint succeeds so an older/deployed backend
      // cannot leave the customer stuck at "Waiting for Admin Approval".
      const qRef=doc(db,'smv_questions',id);
      const qSnap=await withTimeout(getDoc(qRef),15000);
      if(!qSnap.exists()) throw new Error('Question was approved but the Firestore question record could not be found.');
      const qNow=qSnap.data()||{};
      const approvalData={
        status:'admin_approved',
        allocationStatus:'assigned_to_astrologer',
        astrologerId:astroId,
        astrologerName:astro.name||qNow.astrologerName||'Astrologer',
        commissionPercent:pct,
        commissionRate:pct,
        adminQuestionApprovedAt:serverTimestamp(),
        adminQuestionApprovedBy:currentUser.uid,
        updatedAt:serverTimestamp()
      };
      await withTimeout(updateDoc(qRef,approvalData),15000);

      // Replace the customer's old payment/waiting notification for this exact
      // question. If it does not exist, create the correct approval notification.
      const customerId=String(qNow.customerId||'');
      if(customerId){
        let changed=false;
        try{
          const ns=await withTimeout(getDocs(query(collection(db,'smv_notifications'),where('userId','==',customerId))),15000);
          for(const nd of ns.docs){
            const n=nd.data()||{};
            const paymentId=String(qNow.customerPaymentId||'').trim();
            const notificationQuestionId=String(n.questionId||'').trim();
            const notificationText=String(n.message||'');
            const matchesExactQuestion=notificationQuestionId===String(id);
            // Older payment notifications were created without questionId.
            // Match those safely using the exact Customer Payment ID shown in
            // the notification (for example SMV-PAY-31082026-03).
            const matchesPaymentId=!!paymentId && notificationText.includes(paymentId);
            const oldText=notificationText.toLowerCase();
            const oldTitle=String(n.title||'').toLowerCase();
            if((matchesExactQuestion || matchesPaymentId) && (n.type==='payment_success' || n.type==='payment_verified' || oldTitle.includes('payment successful') || oldText.includes('waiting for admin approval'))){
              await updateDoc(nd.ref,{
                type:'question_approved',
                title:'Question Approved',
                message:'Your question has been approved by Admin and allocated to '+(astro.name||qNow.astrologerName||'the selected astrologer')+'.',
                questionId:id,
                astrologerId:astroId,
                astrologerName:astro.name||qNow.astrologerName||'Astrologer',
                updatedAt:serverTimestamp(),
          ...
          console.warn('Customer approval notification reconciliation skipped:',notificationReadError);
        }
        if(!changed){
          // Final fallback: create one canonical approval notification. This is
          // intentionally tied to the exact question/payment so the Customer
          // Dashboard can never remain stuck on the old payment message.
          await setDoc(doc(db,'smv_notifications',customerId+'_question_approved_'+id+'_'+Date.now()),{
            userId:customerId,
            type:'question_approved',
            title:'Question Approved',
            message:'Your question has been approved by Admin and allocated to '+(astro.name||qNow.astrologerName||'the selected astrologer')+'.',
      ...
      alert('Question approved and allocated to '+(astro.name||'the selected astrologer')+'. Customer Dashboard is now updated.');
      await loadAdminPanel();
    } catch(e){
      console.error('QUESTION APPROVAL SYNC ERROR:',e);
      alert(e.message||String(e));
      b.disabled=false; b.textContent='APPROVE & ALLOCATE';
    }
  });

  questionApprovalBox.querySelectorAll('[data-board-keep]').forEach(b=>b.onclick=()=>{b.textContent='KEPT ALLOCATED';b.disabled=true;alert('Question remains allocated to the current astrologer and stays in the Admin Question Board until an answer is finally submitted and approved.');});
  questionApprovalBox.querySelectorAll('[data-board-realloc]').forEach(b=>b.onclick=async()=>{
    const id=b.dataset.boardRealloc; const astroId=$('boardReallocAstro_'+id)?.value||''; const pct=Number($('boardReallocPct_'+id)?.value);
    ...
    b.disabled=true; b.textContent='RE-ALLOCATING...';
    try{
      const r=await renderApi('/admin/reallocate-question',{method:'POST',body:JSON.stringify({questionId:id,astrologerId:astroId,commissionPercent:pct})});
      if(!r?.success) throw new Error(r?.error||'Unable to re-allocate question.');
      alert('Question re-allocated successfully. Same Question ID retained.'); await loadAdminPanel();
    }catch(e){alert(e.message||String(e));b.disabled=false;b.textContent='RE-ALLOCATE';}
  });
  questionApprovalBox.querySelectorAll('[data-board-admin-answer]').forEach(b=>b.onclick=async()=>{
    const id=b.dataset.boardAdminAnswer; const answer=($('boardAdminAnswer_'+id)?.value||'').trim();
    if(!answer){smvNotice('Admin Answer','Enter the Admin answer before submitting.','!');return;}
    b.disabled=true; b.textContent='ANSWERING...';
    try{
      ...
      smvNotice('Answer Submitted','Admin answer submitted successfully. Full payment remains with Admin.','✓'); await loadAdminPanel();
    }catch(e){smvNotice('Unable to Submit',e.message||String(e),'!');b.disabled=false;b.textContent='ADMIN ANSWER';}
  });
  questionApprovalBox.querySelectorAll('[data-reject-question]').forEach(b=>b.onclick=async()=>{
    ...
    b.disabled=true; b.textContent='REJECTING & REFUNDING...';
    try{
      const r=await renderApi('/admin/reject-question',{method:'POST',body:JSON.stringify({questionId:id,reason})});
      if(!r?.success) throw new Error(r?.error||'Unable to reject question and process refund.');
      const msg=r.refundStatus==='processed'||r.refundStatus==='completed'
        ? 'Question rejected. Refund completed. Refund ID: '+(r.refundId||'')
        : r.refundStatus==='failed'
          ? 'Question rejected, but the automatic refund could not be created. Admin must review the refund.'
          : 'Question rejected. Refund is '+(r.refundStatus||'pending')+'.';
      alert(msg);
      await loadAdminPanel();
    }catch(e){alert(e.message||String(e));b.disabled=false;b.textContent='REJECT QUESTION';}
  });
  // Admin refund monitor: rejected paid questions remain visible here until the
  // Razorpay refund reaches a terminal state. Admin can force a live status sync.
  const adminRefundBox=$('adminRefunds');
  try{
    const refundQuestions=questions.docs.filter(d=>{const q=d.data()||{};return ['question_rejected','admin_rejected'].includes(String(q.status||'')) && Number(q.refundAmount||q.amount||q.paymentAmount||0)>0;});
    adminRefundBox.innerHTML=refundQuestions.length?refundQuestions.map(d=>{
      const q=d.data()||{}; const rs=String(q.refundStatus||'pending').toLowerCase();
      const terminal=['processed','completed','failed'].includes(rs);
      const badge=rs==='failed'?'<span class="error"><b>🔴 Refund Failed</b></span>':(['processed','completed'].includes(rs)?'<span class="success"><b>🟢 Refund Completed</b></span>':'<span style="color:#b26a00"><b>🟠 Refund Pending</b></span>');
      return `<div class="card" style="margin:10px 0"><h3>${escapeHtml(q.customerName||q.birthName||'Customer')}</h3><div class="small"><b>Question ID:</b> ${escapeHtml(d.id)} · <b>Paid:</b> ₹${Number(q.amount||q.paymentAmount||q.refundAmount||0).toFixed(2)}</div><div class="small"><b>Payment ID:</b> ${escapeHtml(q.customerPaymentId||q.razorpayPaymentId||'')}</div><div class="small"><b>Question Status:</b> 🔴 Question Rejected · <b>Refund:</b> ${badge}</div><div class="small"><b>Refund Amount:</b> ₹${Number(q.refundAmount||q.amount||q.paymentAmount||0).toFixed(2)} · <b>Refund ID:</b> ${escapeHtml(q.refundId||'Not created')}</div>${q.refundReason||q.adminQuestionRejectionReason?`<div class="small"><b>Reason:</b> ${escapeHtml(q.refundReason||q.adminQuestionRejectionReason)}</div>`:''}${q.refundProcessedAt?`<div class="small"><b>Refund Date:</b> ${escapeHtml(smvDateTime(q.refundProcessedAt))}</div>`:''}${q.refundError?`<div class="small error"><b>Refund Error:</b> ${escapeHtml(q.refundError)}</div>`:''}${!terminal&&q.refundId?`<div class="action-row" style="margin-top:8px"><button class="btn gray" data-sync-refund="${escapeHtml(d.id)}">SYNC RAZORPAY REFUND</button></div>`:''}</div>`;
    ...
    adminRefundBox.querySelectorAll('[data-sync-refund]').forEach(b=>b.onclick=async()=>{
      ...
      try{const r=await withTimeout(renderApi('/admin/sync-refund',{method:'POST',body:JSON.stringify({questionId:id})}),20000);if(!r?.success)throw new Error(r?.error||'Unable to sync refund status.');alert('Razorpay refund status: '+(r.refundStatus||'unknown')+(r.refundId?'\nRefund ID: '+r.refundId:''));await loadAdminPanel();}
      catch(e){alert(e.message||String(e));b.disabled=false;b.textContent='SYNC RAZORPAY REFUND';}
    });
  }catch(e){console.warn('Admin refund monitor failed:',e);if(adminRefundBox)adminRefundBox.innerHTML='<div class="empty error">Unable to load refund monitor: '+escapeHtml(e.message||String(e))+'</div>';}
    ...
    ? pendingAnswers.map(d => {
        ...
        return `
          <div class="card" style="margin:10px 0">
            ...
            <b>Question ID: ${escapeHtml(d.id)}</b><br><b>
  ...
  .forEach(b => {
    ...
    b.onclick = async () => {
      ...
      try {
        ...
        const approvalResult = await renderApi('/admin/approve-answer',{
          method:'POST',
          body:JSON.stringify({questionId:id})
        });
        if(!approvalResult?.success) throw new Error(approvalResult?.error||'Unable to approve answer.');

        const commissionAmount = Number(approvalResult.commissionAmount || q.astrologerCommissionAmount || 0);

        // Email delivery is handled silently in the backend.
        // Customer/Astrologer web UI must show only the business status,
        // never Resend/email delivery errors.
        alert(
          approvalResult.already
            ? `Answer approval is already completed. ₹${commissionAmount.toFixed(2)} commission remains credited.`
