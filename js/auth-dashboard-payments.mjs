...
async function loadAdminPanel(){
 ...
 try{
  ...
  try{
    ...
    adminRefundBox.querySelectorAll('[data-retry-refund]').forEach(b=>b.onclick=async()=>{
      const id=b.dataset.retryRefund;
      if(!confirm('Retry this Razorpay refund? The server will first check the original payment to prevent a duplicate refund.')) return;
      b.disabled=true; b.textContent='RETRYING...';
      try{const r=await withTimeout(renderApi('/admin/retry-refund',{method:'POST',body:JSON.stringify({questionId:id})}),25000);if(!r?.success)throw new Error(r?.error||'Unable to retry refund.');alert('Refund retry: '+(r.refundStatus||'unknown')+(r.refundId?'\nRefund ID: '+r.refundId:'')+(r.alreadyFullyRefunded?'\nRazorpay already shows this payment as fully refunded.':''));await loadAdminPanel();}
      catch(e){alert(e.message||String(e));b.disabled=false;b.textContent='RETRY REFUND';}
    });
