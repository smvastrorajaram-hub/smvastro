import {createRefreshQueue,watchDashboardEvents} from "./dashboard-live.mjs?v=13";
...
<div class="card" style="margin-top:16px"><h3>Payment Method</h3><p class="small">Your bank/UPI details are private. Full details are not displayed again.</p><button class="btn gray" id="changePayoutBtn2">Change Payment Method</button></div>`;
  ...
  try{
    ...
    histBox.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;gap:10px"><h3 style="margin:0">Private Consultation History</h3><button type="button" id="privateConsultHistoryToggle" aria-label="Minimize or expand Private Consultation History" style="border:0!important;background:none!important;background-color:transparent!important;box-shadow:none!important;outline:none;padding:0;margin:0;font-size:20px;line-height:1;cursor:pointer;color:inherit;appearance:none;-webkit-appearance:none">▼</button></div><div id="privateConsultHistoryBody" style="margin-top:12px">${historyItems.length?historyItems.map(c=>{
  ...
  try {
   ...
   <div class="card" style="margin-top:16px"><h3>My Private Consultations</h3>${!privateConsultationItems.length?'<div class="empty">No private consultations yet.</div>':privateConsultationItems.slice(0,20).map(c=>{const id=String(c.id||c.consultationId||'');const rejected=c.status==='question_rejected';const refundStatus=String(c.refundStatus||'').toLowerCase();const refundDone=['processed','completed'].includes(refundStatus);const answerReady=c.status==='answered'&&!!String(c.answer||'').trim();const unpaid=c.paymentStatus!=='paid'&&!rejected;const statusMap={awaiting_payment:'Payment Pending',pending_admin_approval:'Waiting for Admin Approval',approved_for_astrologer:'Waiting for Selected Astrologer',answer_pending_admin_approval:'Answer Waiting for Admin Approval',revision_required:'Astrologer Revising Answer',answered:c.customerViewedAt?'Answer Viewed':'Answer Ready',question_rejected:'Question Rejected / Refund'};return `<div class="smv-consultation-item" style="padding:14px 0;border-bottom:1px solid #eee"><b>${escapeHtml(c.question||'Private Consultation')}</b><div class="small"><b>Selected Astrologer:</b> ${escapeHtml(c.astrologerName||'Astrologer')} · <b>Chat Price:</b> ₹${Number(c.chatPrice||c.amount||0).toFixed(2)}</div><div class="small"><b>Status:</b> ${escapeHtml(statusMap[c.status]||c.status||'Processing')}</div><div class="small"><b>Payment Status:</b> ${escapeHtml(c.paymentStatus||'pending')}</div><div class="small"><b>Consultation ID:</b> ${escapeHtml(id)}</div>${c.razorpayPaymentId?`<div class="small"><b>Payment ID:</b> ${escapeHtml(c.razorpayPaymentId)}</div>`:''}${unpaid?`<div style="margin-top:8px"><button class="btn" data-private-retry="${escapeHtml(id)}">RETRY PAYMENT ₹${Number(c.chatPrice||c.amount||0).toFixed(2)}</button> <button class="btn gray" data-private-recover="${escapeHtml(id)}">RECOVER PAYMENT</button></div>`:''}${rejected?`<div class="refund-summary" style="margin-top:10px"><b>Refund Status:</b> ${refundDone?'Refund Completed':(refundStatus==='failed'?'Refund Failed — Admin Retry Required':escapeHtml(c.refundStatus||'Pending'))}<br><span class="small"><b>Refund Amount:</b> ₹${Number(c.refundAmount||c.chatPrice||0).toFixed(2)}</span>${c.refundId?`<br><span class="small"><b>Refund ID:</b> ${escapeHtml(c.refundId)}</span>`:''}<br><span class="small"><b>RRN:</b> ${escapeHtml(c.refundRrn||'Available after Razorpay processes the refund')}</span>${c.refundProcessedAt?`<br><span class="small"><b>Refund Date:</b> ${escapeHtml(smvDateTime(c.refundProcessedAt))}</span>`:''}</div>`:''}${answerReady?`<div class="card" style="margin-top:10px"><b>Astrologer Answer</b><p style="white-space:pre-wrap;overflow-wrap:anywhere">${escapeHtml(c.answer)}</p>${c.customerViewedAt?'<div class="small success">SUBMITTED / CUSTOMER VIEWED</div>':`<button class="btn" data-private-view="${escapeHtml(id)}">MARK ANSWER VIEWED</button>`}</div>`:''}</div>`}).join('')}</div>`;
 ...
 try{
  ...
  const renderQuizManager=async()=>{
    try{
      ...
      qm.innerHTML=`<div style="display:flex;align-items:center;gap:8px"><button type="button" id="astrologerQuizQuestionManagerToggle" aria-label="Collapse 25 Question Section" title="Collapse" style="border:0!important;background:none!important;background-color:transparent!important;box-shadow:none!important;outline:none!important;padding:0!important;margin:0!important;font-size:18px;line-height:1;cursor:pointer;color:inherit!important;appearance:none;-webkit-appearance:none">▼</button><h3 style="margin:0">Astrologer Qualification Test — Question Manager</h3></div><div id="astrologerQuizQuestionManagerBody"><p class="small">Edit Tamil + English question, four choices, correct answer, order and Enable/Disable. Only enabled questions are sent to Google Form. Pass Mark is controlled above.</p><div class="action-row"><button class="btn" id="quizLoadDefaults">LOAD DEFAULT 25 QUESTIONS</button><button class="btn" id="quizAddQuestion">ADD QUESTION</button><button class="btn" id="quizSyncGoogleForm">SYNC GOOGLE FORM</button></div><div id="quizManagerMsg" class="small"></div><div id="quizQuestionList"></div></div>`;
