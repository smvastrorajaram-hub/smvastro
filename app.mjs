...
async function loadDashboard(expectedRole=null,force=false,background=false){
 ...
 try{
  ...
  const customerRequest=requestedRole==='customer'?renderApi('/customer/consultations?_fresh='+Date.now()+'-'+loadId,{method:'GET'}).then(value=>({value}),error=>({error})):null;
...
<div class="card" style="margin-top:16px"><h3>Payment Method</h3><p class="small">Your bank/UPI details are private. Full details are not displayed again.</p><button class="btn gray" id="changePayoutBtn2">Change Payment Method</button></div>`;
  ...
  try {
   ...
   const cr=customerResult?customerResult.value:await renderApi('/customer/consultations?_fresh='+Date.now()+'-'+loadId,{method:'GET'});
